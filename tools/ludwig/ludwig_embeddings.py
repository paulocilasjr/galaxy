...
def get_image_files_from_zip(zip_file):
    """Returns a list of valid image file names in the ZIP file, excluding directories."""
    try:
        with zipfile.ZipFile(zip_file, "r") as zip_ref:
            # Validate ZIP entries before adding to file_list
            file_list = []
            for f in zip_ref.namelist():
                if f.lower().endswith((".png", ".jpg", ".jpeg", ".bmp", ".gif")) and not f.endswith("/"):
                    try:
                        # Test opening the file to ensure it's valid
                        with zip_ref.open(f) as test_file:
                            test_file.read(1)  # Read a byte to check validity
                        file_list.append(f)
                    except zipfile.BadZipFile as e:
                        logging.warning("Skipping invalid ZIP entry: %s (%s)", f, e)
                    except Exception as e:
                        logging.warning("Skipping file: %s (%s)", f, e)
            if not file_list:
                logging.warning("No valid image files found in ZIP")
            logging.info("Valid image files listed from ZIP: %s", file_list)
            return file_list
    except zipfile.BadZipFile as exc:
        logging.error("Invalid ZIP file: %s", exc)
        raise RuntimeError("Invalid ZIP file.") from exc
    except Exception as exc:
        logging.error("Error reading ZIP file: %s", exc)
...
def load_model(model_name, device):
    ...
    if model_name not in AVAILABLE_MODELS:
        raise ValueError(
            f"Unsupported model: {model_name}. Available models: {list(AVAILABLE_MODELS.keys())}")
    try:
        ...
        logging.error(f"Failed to load model {model_name}: %s", e)
        raise
...
def extract_embeddings(model_name, apply_normalization, zip_file, file_list, transform_type="rgb"):
    """Extracts embeddings from images using batch processing or sequential fallback."""
    ...
    if transform_type.lower() == "grayscale":
        initial_transform = transforms.Grayscale(num_output_channels=3)
    elif transform_type.lower() == "clahe":
        initial_transform = CLAHETransform()
    elif transform_type.lower() == "edges":
        initial_transform = CannyTransform()
    elif transform_type.lower() == "rgba_to_rgb":
        ...
        def __getitem__(self, idx):
            try:
                with zipfile.ZipFile(self.zip_file, "r") as zip_ref:
                    ...
                    logging.debug("Attempting to process file: %s", file_path)
                ...
                logging.warning("Bad ZIP file error for %s: %s", self.file_list[idx], e)
                return None, os.path.basename(self.file_list[idx])
            except Exception as e:
                logging.warning("Skipping %s: %s", self.file_list[idx], e)
                return None, os.path.basename(self.file_list[idx])
    ...
    # Determine number of workers based on GALAXY_SLOTS
    galaxy_slots = os.environ.get('GALAXY_SLOTS')
    if galaxy_slots:
        try:
            num_cpus = int(galaxy_slots)
            num_workers = max(0, num_cpus - 1)
        except ValueError:
            logging.warning("Invalid GALAXY_SLOTS value: %s. Defaulting to single-threaded.", galaxy_slots)
            num_workers = 0
    else:
        logging.info("GALAXY_SLOTS not set. Defaulting to single-threaded.")
        num_workers = 0
    logging.info("Using %d workers for DataLoader (GALAXY_SLOTS: %s)", num_workers, galaxy_slots or "Not set")

    list_embeddings = []
    with torch.inference_mode():
        try:
            # Try DataLoader with dynamic worker count
            ...
            logging.warning(f"DataLoader failed: {e}. Falling back to sequential processing.")
            # Fallback to sequential processing
            for file in file_list:
                try:
                    with zipfile.ZipFile(zip_file, "r") as zip_ref:
                        logging.debug("Processing file (sequential): %s", file)
                        with zip_ref.open(file) as img_file:
                            ...
                            logging.info("Successfully processed (sequential): %s", file)
                except zipfile.BadZipFile as e:
                    logging.warning("Bad ZIP file error for %s: %s", file, e)
                except Exception as e:
